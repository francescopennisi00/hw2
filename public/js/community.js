...
function onPostsJson(json) {
    ...
    for(const post of json) {
        ...
        fetch(BASE_URL + "/community/post/verifica_like/"+encodeURIComponent(idpost)).then(onResponse).then(onVerLikeJson);
...
function aggiornaPost() {
    fetch(BASE_URL + "/community/getposts").then(onResponse).then(onPostsJson);  
}
